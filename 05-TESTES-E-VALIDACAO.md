...
| **Formulário mostra o front antigo (precisa Ctrl+F5)** | Cache do browser/túnel segurava CSS/JS | Corrigido por cache-bust automático no `server.js`: HTML com `Cache-Control: no-cache` + `?v=` (hash do mtime) nos assets. Recarregar normal já traz o novo. |
| **Botão "Reiniciar Gateway" falha no server** | O Node do painel (systemd `--user`) não herda `DBUS_SESSION_BUS_ADDRESS`/`XDG_RUNTIME_DIR` | Corrigido em `services/sshRunner.js` (`restartHermesGateway`), que injeta o bus (`/run/user/$(id -u)`) antes do `systemctl --user restart`. |
| **Botão "Reiniciar Gateway" falha no Windows** | Shell do Windows OpenSSH é PowerShell 5.1, que não suporta `&&` | Corrigido em `services/sshRunner.js`: a cadeia `hermes gateway stop && schtasks` é envolta em `cmd /c "…"` para o `cmd.exe` interpretar. |
